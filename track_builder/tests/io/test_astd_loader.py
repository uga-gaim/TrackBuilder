...
def test_load_astd_data_single_file(tmp_path: Path):
    ...
    df = load_astd_data(f, progress=False, use_optimized_config=False)
    assert len(df) == 1
...
def test_load_astd_data_multiple_files_auto_merge(tmp_path: Path):
    ...
    df = load_astd_data([f1, f2], progress=False, use_optimized_config=False)
    assert len(df) == 2
...
def test_load_astd_data_directory_with_pattern(tmp_path: Path):
    ...
    df = load_astd_data(d, pattern="ASTD_*.csv", progress=False, use_optimized_config=False)
    assert len(df) == 2
...
def test_load_astd_monthly_specific_months(tmp_path: Path):
    ...
    df = load_astd_monthly(tmp_path, 2019, months=[7, 9], progress=False, use_optimized_config=False)
...
def test_load_astd_monthly_full_year(tmp_path: Path):
    ...
    df = load_astd_monthly(
        tmp_path, 2019, progress=False, use_optimized_config=False
    )
